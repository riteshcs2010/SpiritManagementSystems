...
Public Class frmSqlServerSetting
            ...
            If (con.State = ConnectionState.Open) Then
                If MsgBox("It will create the DB and configure the sql server, Do you want to proceed?", MsgBoxStyle.YesNo + MsgBoxStyle.Information) = MsgBoxResult.Yes Then
                    ...
                    Using sw As StreamWriter = New StreamWriter(Application.StartupPath & "\SQLSettings.dat")
                        If cmbAuthentication.SelectedIndex = 0 Then
                            sw.WriteLine(Encrypt("Data Source=" & cmbServerName.Text.Trim & ";Initial Catalog=SIS_DB;Integrated Security=True"))
                            sw.Close()
                        End If
                        If cmbAuthentication.SelectedIndex = 1 Then
                            sw.WriteLine(Encrypt("Data Source=" & cmbServerName.Text.Trim & ";Initial Catalog=SIS_DB;User ID=" & txtUserName.Text.Trim & ";Password=" & txtPassword.Text & ""))
                            sw.Close()
                        End If
                        CreateDB()
                    End Using
    ...
    Private Sub btnTestConnection_Click_1(sender As System.Object, e As System.EventArgs) Handles btnTestConnection.Click
        ...
        If SqlConn.State = ConnectionState.Closed Then
            ...
            Try
                SqlConn.Open()
                MessageBox.Show("Server Connnection Successful", "Connection Test", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Catch ex As Exception
                MessageBox.Show("Server Connection Failed" + vbCrLf + Err.Description, "Test Failed", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End Try
        End If
    End Sub
    ...
    Private Sub btnBlankDataDB_Click(sender As System.Object, e As System.EventArgs) Handles btnBlankDataDB.Click
        Try
            ...
            con.Open()
            If (con.State = ConnectionState.Open) Then
                If MsgBox("This will save the server settings. Do you want to proceed?", MsgBoxStyle.YesNo + MsgBoxStyle.Information) = MsgBoxResult.Yes Then

                    Using sw As StreamWriter = New StreamWriter(Application.StartupPath & "\SQLSettings.dat")
                        If cmbAuthentication.SelectedIndex = 0 Then
                            sw.WriteLine(Encrypt("Data Source=" & cmbServerName.Text.Trim & ";Initial Catalog=SIS_DB;Integrated Security=True;MultipleActiveResultSets=True"))
                            sw.Close()
                        End If
                        If cmbAuthentication.SelectedIndex = 1 Then
                            sw.WriteLine(Encrypt("Data Source=" & cmbServerName.Text.Trim & ";Initial Catalog=SIS_DB;User ID=" & txtUserName.Text.Trim & ";Password=" & txtPassword.Text & ";MultipleActiveResultSets=True"))
                            sw.Close()
                        End If
                        'CreateBlankDB()
                        MessageBox.Show("Server settings saved" & vbCrLf & "Application will now close. Please re-launch again.", "", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        End
                    End Using
                Else
                    End
            ...
            End If
        Catch ex As Exception
            MessageBox.Show("Unable to connect to server" + vbCrLf + Err.Description, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        Finally
            If (con.State = ConnectionState.Open) Then
                con.Close()
            End If
