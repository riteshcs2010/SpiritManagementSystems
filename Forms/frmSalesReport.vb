...
Public Class frmSalesReport
    ...
    Private Sub Button1_Click(sender As System.Object, e As System.EventArgs) Handles Button1.Click
        Try
            ...
            MyCommand.CommandText = "SELECT SalesUnit, Customer.ID, Customer.Name, Customer.Gender, Customer.Address, Customer.City, Customer.State, Customer.ZipCode, Customer.ContactNo, Customer.EmailID, Customer.Remarks,Customer.Photo, InvoiceInfo.Inv_ID, InvoiceInfo.InvoiceNo, InvoiceInfo.InvoiceDate, InvoiceInfo.CustomerID , InvoiceInfo.GrandTotal, InvoiceInfo.TotalPaid, InvoiceInfo.Balance, Invoice_Product.IPo_ID, Invoice_Product.InvoiceID, Invoice_Product.ProductID, Invoice_Product.CostPrice, Invoice_Product.SellingPrice, Invoice_Product.Margin,Invoice_Product.Qty, Invoice_Product.Amount, Invoice_Product.DiscountPer, Invoice_Product.Discount, Invoice_Product.VATPer, Invoice_Product.VAT, Invoice_Product.TotalAmount, Product.PID,Product.ProductCode, Product.ProductName FROM Customer INNER JOIN InvoiceInfo ON Customer.ID = InvoiceInfo.CustomerID INNER JOIN Invoice_Product ON InvoiceInfo.Inv_ID = Invoice_Product.InvoiceID INNER JOIN Product ON Invoice_Product.ProductID = Product.PID INNER JOIN  Company on Company.ID=Product.CompanyId where InvoiceDate between @d1 and @d2 and Company.CompanyName= @d3 order by InvoiceDate"

            MyCommand.Parameters.Add("@d1", SqlDbType.DateTime, 30, "Date").Value = dtpDateFrom.Value.Date
            MyCommand.Parameters.Add("@d2", SqlDbType.DateTime, 30, "Date").Value = dtpDateTo.Value.Date
            MyCommand.Parameters.Add("@d3", SqlDbType.VarChar, 30, "String").Value = cmbCompany.Text


            MyCommand1.CommandText = "SELECT * from Company where CompanyName=@d1"
            MyCommand1.Parameters.Add("@d1", SqlDbType.VarChar, 30, "String").Value = cmbCompany.Text
    ...
    Sub fillCompany()
        Try
            ...
            adp.SelectCommand = New SqlCommand("SELECT distinct RTRIM(CompanyName) FROM dbo.Company where companygroupid=@d1 order by 1 ", con)
            adp.SelectCommand.Parameters.AddWithValue("@d1", GlobalCompanygroupID)
