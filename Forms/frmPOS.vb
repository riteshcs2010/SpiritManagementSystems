...
Public Class frmPOS
    ...
    Sub ComputePayment(ByVal Amount As Decimal)

        txtPayment.Text = Amount
    ...
    Private Sub cmbPaymentMode_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cmbPaymentMode.SelectedIndexChanged
        ...
        ElseIf cmbPaymentMode.SelectedIndex <> 0 Then
    ...
    Public Sub FillSalesMandata()
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT TOP 1 SM_ID,RTRIM(Salesman_ID),RTRIM([Name]), RTRIM(Address),RTRIM(City),RTRIM(State),RTRIM(ZipCode), RTRIM(ContactNo), RTRIM(EmailID),CommissionPer,RTRIM(Remarks),Photo from Salesman order by Salesman_ID", con)
            rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            'dgw.Rows.Clear()
            While (rdr.Read() = True)
                txtSM_ID.Text = rdr(0)
                txtSalesmanID.Text = rdr(1)
                txtSalesman.Text = rdr(2)
    ...
    Public Sub FillCustomerData()
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT top 1 RTRIM(ID),RTRIM(CustomerID),RTRIM([Name]),RTRIM(Gender), RTRIM(Address),RTRIM(City),RTRIM(State),RTRIM(ZipCode), RTRIM(ContactNo), RTRIM(EmailID),RTRIM(Remarks),Photo from Customer where CustomerType='Regular' order by CustomerID", con)
            rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            'dgw.Rows.Clear()
            While (rdr.Read() = True)
                txtCustomerID.Text = rdr(1)
                txtCustomerName.Text = rdr(2)
                'dgw.Rows.Add(rdr(0), rdr(1), rdr(2), rdr(3), rdr(4), rdr(5), rdr(6), rdr(7), rdr(8), rdr(9), rdr(10), rdr(11))
            End While
            con.Close()
        Catch ex As Exception
    ...
    Private Sub frmPOS_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If frmCustomerRecord2.lblSet.Text <> "Billing" Then

            'cmbPaymentMode.Enabled = False
            txtContactNo.Text = "999999999"

            FillCustomerData()

        End If
        If frmSalesmanRecord.lblSet.Text <> "Billing" Then
